...
void init_TIM(void)
{
    ...
    configTIM.filter.inputPin = &IfxGtm_TIM0_0_P15_6_IN;                                /* Select input port pin                    */
    configTIM.filter.inputPinMode = IfxPort_InputMode_pullDown;         /* Select input port pin mode               */
    IfxGtm_Tim_In_init(&g_driverTIM[0], &configTIM);                       /* Initialize the TIM                       */
    //IOC1
    IfxGtm_Tim_In_initConfig(&configTIM, &MODULE_GTM);                  /* Initialize default parameters            */
    configTIM.filter.inputPin = &IfxGtm_TIM0_1_P15_7_IN;                                /* Select input port pin                    */
    configTIM.filter.inputPinMode = IfxPort_InputMode_pullDown;         /* Select input port pin mode               */
    IfxGtm_Tim_In_init(&g_driverTIM[1], &configTIM);                       /* Initialize the TIM */
    //IOC2
    IfxGtm_Tim_In_initConfig(&configTIM, &MODULE_GTM);                  /* Initialize default parameters            */
    configTIM.filter.inputPin = &IfxGtm_TIM0_2_P15_8_IN;                                /* Select input port pin                    */
    configTIM.filter.inputPinMode = IfxPort_InputMode_pullDown;         /* Select input port pin mode               */
    IfxGtm_Tim_In_init(&g_driverTIM[2], &configTIM);                       /* Initialize the TIM                       */
    //IOC3
    IfxGtm_Tim_In_initConfig(&configTIM, &MODULE_GTM);                  /* Initialize default parameters            */
    configTIM.filter.inputPin = &IfxGtm_TIM0_3_P21_5_IN;                                /* Select input port pin                    */
    configTIM.filter.inputPinMode = IfxPort_InputMode_pullDown;         /* Select input port pin mode               */
    IfxGtm_Tim_In_init(&g_driverTIM[3], &configTIM);                       /* Initialize the TIM */
}
...
void measure_PWM(void)
{
    for(int i = 0; i < 4; i++)
    {
        IfxGtm_Tim_In_update(&g_driverTIM[i]);                                         /* Update the measured data         */
        g_measuredPwmPeriod[i] = IfxGtm_Tim_In_getPeriodSecond(&g_driverTIM[i]);          /* Get the period of the PWM signal */
        g_measuredPwmFreq_Hz[i] = 1 / g_measuredPwmPeriod[i];                             /* Calculate the frequency          */
        g_measuredPwmDutyCycle[i] = IfxGtm_Tim_In_getDutyPercent(&g_driverTIM[i], &g_dataCoherent); /* Get the duty cycle     */
    }
}
