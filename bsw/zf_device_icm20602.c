...
sint16 icm20602_acc_x  = 0, icm20602_acc_y  = 0, icm20602_acc_z  = 0;                    // 三轴加速度计数据     acc (accelerometer 加速度计)

float32 acc_x = 0, acc_y = 0, acc_z = 0;
float32 gyro_x = 0, gyro_y = 0, gyro_z = 0;

static void system_delay_ms(uint32 times)
{
    Ifx_TickTime ticksFor1ms = IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, 1);
    waitTime(ticksFor1ms * times);    /* Wait 100 ms */
}
...
void icm20602_get_acc (void)
{
    ...
    icm20602_acc_x = (sint16)(((uint16)dat[0] << 8 | dat[1]));
    icm20602_acc_y = (sint16)(((uint16)dat[2] << 8 | dat[3]));
    icm20602_acc_z = (sint16)(((uint16)dat[4] << 8 | dat[5]));

    acc_x = icm20602_acc_transition(icm20602_acc_x);
    acc_y = icm20602_acc_transition(icm20602_acc_y);
    acc_z = icm20602_acc_transition(icm20602_acc_z);
}
...
void icm20602_get_gyro (void)
{
    ...
    icm20602_gyro_x = (sint16)(((uint16)dat[0] << 8 | dat[1]));
    icm20602_gyro_y = (sint16)(((uint16)dat[2] << 8 | dat[3]));
    icm20602_gyro_z = (sint16)(((uint16)dat[4] << 8 | dat[5]));

    gyro_x = icm20602_gyro_transition(icm20602_gyro_x);
    gyro_y = icm20602_gyro_transition(icm20602_gyro_y);
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
float32 icm20602_acc_transition (sint16 acc_value)
{
    float32 acc_data = 0;
    switch(ICM20602_ACC_SAMPLE)
    {
        case 0x00: acc_data = (float32)acc_value / 16384; break;      // 0x00 加速度计量程为:±2g          获取到的加速度计数据 除以16384      可以转化为带物理单位的数据，单位：g(m/s^2)
        case 0x08: acc_data = (float32)acc_value / 8192;  break;      // 0x08 加速度计量程为:±4g          获取到的加速度计数据 除以8192       可以转化为带物理单位的数据，单位：g(m/s^2)
        case 0x10: acc_data = (float32)acc_value / 4096;  break;      // 0x10 加速度计量程为:±8g          获取到的加速度计数据 除以4096       可以转化为带物理单位的数据，单位：g(m/s^2)
        case 0x18: acc_data = (float32)acc_value / 2048;  break;      // 0x18 加速度计量程为:±16g         获取到的加速度计数据 除以2048       可以转化为带物理单位的数据，单位：g(m/s^2)
        default: break;
    }
    return acc_data;
}
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
float32 icm20602_gyro_transition (sint16 gyro_value)
{
    float32 gyro_data = 0;
    switch(ICM20602_GYR_SAMPLE)
    {
        case 0x00: gyro_data = (float32)gyro_value / 131.2f;  break;  //  0x00 陀螺仪量程为:±250 dps     获取到的陀螺仪数据除以131           可以转化为带物理单位的数据，单位为：°/s
        case 0x08: gyro_data = (float32)gyro_value / 65.6f;   break;  //  0x08 陀螺仪量程为:±500 dps     获取到的陀螺仪数据除以65.5          可以转化为带物理单位的数据，单位为：°/s
        case 0x10: gyro_data = (float32)gyro_value / 32.8f;   break;  //  0x10 陀螺仪量程为:±1000dps     获取到的陀螺仪数据除以32.8          可以转化为带物理单位的数据，单位为：°/s
        case 0x18: gyro_data = (float32)gyro_value / 16.4f;   break;  //  0x18 陀螺仪量程为:±2000dps     获取到的陀螺仪数据除以16.4          可以转化为带物理单位的数据，单位为：°/s
        default: break;
    }
    return gyro_data;
}
